    ...
    >
      <p
        className={cn(
          ...
          isCancelled ? "text-muted-foreground" : "text-red-500",
        )}
      >
        {headerText}
      </p>
      <p
        className={cn(
          ...
          isCancelled ? "text-muted-foreground" : "text-red-500/90",
        )}
      >
        {errorText}
      </p>
...
const ToolFallbackImpl = ({
  ...
  const isCancelled =
    status?.type === "incomplete" && status.reason === "cancelled";
  const hasResultError = !!getResultErrorText(result);
  const isFailed = hasResultError || (status?.type === "incomplete" && !isCancelled);
  ...
  return (
    ...
    >
      ...
      <ToolFallbackContent>
        <ToolFallbackError status={status} result={result} />
        <ToolFallbackArgs
          argsText={argsText}
...
const ToolFallback = memo(
  ToolFallbackImpl,
) as unknown as React.FC<ToolFallbackImplProps> & {
  displayName?: string;
