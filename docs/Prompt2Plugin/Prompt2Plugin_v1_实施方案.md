...
同专题另有：[开发目标](./Prompt2Plugin_开发目标.md)、[通用提示词规范](./Prompt2Plugin_通用提示词规范_v1.md)、[索引](./README.md)。
...
### 3.1 目录职责示意图（与 `README` 对齐）

源码树、宿主工作区、一次性校验空间的职责分界见 [README.md § 目录职责示意图](./README.md#目录职责示意图插件侧-vs-宿主侧)。要点：

1. `**plugins/<plugin-id>/**`：唯一稳定挂载点，`promote` 前不得被生成流程直接覆盖。
2. `**plugins/.drafts/<plugin-id>/**`：草稿与校验链读写边界；revision 等与 `.p2p-meta.json` 对齐。
3. `**var/plugin-workspaces/<plugin-id>/**`（相对 host-api 进程 cwd）：运行时落盘数据，晋升包体不必包含此目录。
4. **校验／沙箱临时目录**：只对应当次 `validate`/`test`；失败即中止，不写稳定树。

---

## 3.2 多条版本线与 `decorateSessions`（减上下文）

**意图**：新版本不断迭代时，不依赖单一会话拖动全部历史。`prompt2plugin-studio` 若以 `**multi` 会话**，可用 `**decorateSessions`** 返回多行：**每行对应一条版本线入口**（如某 `plugin-id` 的草稿 `revision`／稳定标记／最近状态摘要）；用户从列表选一进入会话，上下文聚焦该线。**真相源仍为** `.p2p-meta.json` 与 promote／审计记录，列表仅是门户与可读摘要（类比按版本整理的说明文档）。

实现细节（`PluginSessionRow` 的 id、会话与 revision 映射）待在 studio 插件与宿主会话模型中联调时敲定。
