...
| 字段 / 方法 | 说明 |
|-------------|------|
| `PluginTurnContext` | `sessionId`、`message`、`config`、可选 **`argv?: { command, args }`**（宿主已解析命令行时传入）、可选 `emitAssistantDelta`。 |
| `PluginTurnHandleResult` | `text` 必填；可选 **`continue`**（全插件通用：仅当 `true` 时宿主可在同一编排语义下再接 LLM 等；默认 `false`）、**`persist`**（追加落库行）。详见 `docs/项目功能/插件/插件.md`「executeTurn 返回协议」。 |
