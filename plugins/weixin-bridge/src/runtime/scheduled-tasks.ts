...
import { listAccounts, pollStartedAccountsOnce } from "../adapters/openclaw-runtime.js";
import type { IngestBridge, PolledMessage, PublishBridge } from "../types.js";
import { accountSessionId, toNumber } from "./session-state.js";
...
function receiveNewMessages(
  ...
  const lines = messages.map((msg) => msg.text ?? "");
...
async function invokeExternalIngest(
  payload: ExternalUserTurnInput,
  ingestExternalUserTurn: IngestBridge | undefined
...
async function maybeIngestInboundForAccount(
  pluginId: string,
  accountId: string,
  accountMessages: PolledMessage[],
  ingestExternalUserTurn: IngestBridge | undefined
): Promise<void> {
  ...
  const wxReplyTo = last?.userId != null && String(last.userId).length > 0 ? String(last.userId) : undefined;
  const payload: ExternalUserTurnInput = {
    sessionId,
    userText,
...
export function getScheduledTasks(): PluginScheduledTask[] {
  return [
    {
      taskId: "poll-inbox",
      intervalMs: 3000,
...
export async function runScheduledTask(
  pluginId: string,
  taskId: string,
  ctx: PluginScheduledTaskContext,
  publish: PublishBridge,
  bridges: {
    ingestExternalUserTurn?: PluginRuntimeExtensionDeps["ingestExternalUserTurn"];
  }
): Promise<void> {
  ...
  try {
    ...
    console.warn(
      `[weixin-bridge][${pluginId}] pollStartedAccountsOnce failed: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  if (pollInterval < 500) {
