...
async function writeRuntimeEntry() {
  ...
  await writeFile(runtimeEntryMjs, `export { default } from "./runtime.js";\n`, "utf8");
}
...
function startWatch() {
  run("pnpm", ["exec", "tsc", "-w", "-p", tsconfigPath]).catch((error) => {
    console.error("[weixin-bridge] watch failed:", error);
    process.exitCode = 1;
  });
}
...
if (process.argv.includes("--watch")) {
  await writeRuntimeEntry();
  console.log("[weixin-bridge] watch mode started");
  startWatch();
}
