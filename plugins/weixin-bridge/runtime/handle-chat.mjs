import { listAccounts, sendMessage, startAccount, startQr, stopAccount, waitQr } from "../bridge-adapter.mjs";
...
function buildChatContext(pluginId, ctx) {
...
async function handleLogin(c) {
  ...
  pendingLoginByPlugin.set(pluginId, {
...
async function handleSend(c) {
  ...
  if (!to || !text) {
    ...
    return "用法：/send <联系人> <内容>";

  }

  await sendMessage({ accountId, to, text });

  return `已发送（模拟）：${to} <- ${text}`;

}



function handlePlainFallback(c) {
...
export async function handleChatTurn(pluginId, ctx) {
